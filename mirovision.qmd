...
# raw tables 
bookmakers <- dbGetQuery(con, "SELECT * FROM bookmakers")
countries <- dbGetQuery(con, "SELECT * FROM countries")
jurors <- dbGetQuery(con, "SELECT * FROM jurors")
# stan-ready tables
stan_countries <- dbGetQuery(con, "SELECT * FROM stan_countries")
stan_contestants <- dbGetQuery(con, "SELECT * FROM stan_contestants")
stan_shows <- dbGetQuery(con, "SELECT * FROM stan_shows")
stan_points <- dbGetQuery(con, "SELECT * FROM stan_points")
stan_ballots <- dbGetQuery(con, "SELECT * FROM stan_ballots")
...
# fixing datatypes caused by postgres
```{r}
stan_countries <- stan_countries |> mutate(stan_country = as.integer(stan_country))
stan_contestants <- stan_contestants |> mutate(stan_contestant = as.integer(stan_contestant))
stan_points <- stan_points |> 
  mutate(
    stan_ballot = as.integer(stan_ballot),
    stan_show = as.integer(stan_show),
    stan_from_country = as.integer(stan_from_country),
    stan_to_country = as.integer(stan_to_country),
    stan_contestant = as.integer(stan_contestant)
  )
stan_shows <- stan_shows |> mutate(stan_show = as.integer(stan_show))
```
...
if (file_exists("stan/contest-fit.rds")) {
  ...
  contest_fit <-
    contest_model$sample(
      ...
      # reduced numbers so i can run in reasonable time
      chains = 2,
      parallel_chains = 2,
      threads_per_chain = 1,
      iter_warmup = 500,
